...
class Unlocker {
 ...
 public:
  Unlocker(pthread_mutex_t* inLock) { pthread_mutex_unlock(mLock); }
  ~Unlocker() { pthread_mutex_lock(mLock); }
};
...
class Locker {
 ...
 public:
  Locker(pthread_mutex_t* inLock) : mLock(inLock) { pthread_mutex_lock(mLock); }
  ~Locker() { pthread_mutex_unlock(mLock); }
};
...
class Thread {
 public:
  ...
  V& getLocal(size_t i) { return local[localBase + i]; }

  void popLocals() { local.erase(local.end() - fun->NumLocals(), local.end()); }

  // stack ops
  void push(Arg v) { stack.push_back(v); }
  void push(V&& v) { stack.push_back(std::move(v)); }

  void push(O o) { stack.push_back(o); }

  void push(double f) { stack.push_back(f); }
  ...
  V pop() {
    if (stackDepth() == 0) throw errStackUnderflow;
  ...
  void popn(size_t n) {
    if (stackDepth() < n) throw errStackUnderflow;
    stack.erase(stack.end() - n, stack.end());
  }

  void clearStack() { popn(stackDepth()); }
...
class VM {
 public:
  ...
  List* getNil(int inItemType) const {
    return inItemType == itemTypeV ? _nilv() : _nilz();
  }
  Array* getNilArray(int inItemType) const {
    return inItemType == itemTypeV ? _anilv() : _anilz();
  }
  ...
  void addBifHelp(std::string const& str) {
    Locker lock(&gHelpMutex);
    bifHelp.push_back(str);
  }
  void addUdfHelp(std::string const& str) {
    Locker lock(&gHelpMutex);
    udfHelp.push_back(str);
  }

  void addBifHelp(const char* name, const char* mask = nullptr,
  ...
  void addUdfHelp(const char* name, const char* mask = nullptr,
                  const char* help = nullptr) {
    std::string helpstr = name;
    if (mask) {
      helpstr += " @";
  ...
  V def(const char* name, int takes, int leaves, PrimFun pf, const char* help,
        Arg value = 0., bool setNoEach = false);
  V defmcx(const char* name, int numArgs, PrimFun pf, const char* help,
           Arg value = 0.);  // multi channel expanded
  V defautomap(const char* name, const char* mask, PrimFun pf, const char* help,
               Arg value = 0.);  // auto mapped
};
...
enum { scopeUndefined, scopeBuiltIn, scopeWorkspace, scopeLocal, scopeFunVar };
...
class CompileScope : public Object {
 public:
  ...
  virtual int directLookup(Thread& th, P<String> const& inName,
                           size_t& outIndex, V& outBuiltIn);
  virtual int indirectLookup(Thread& th, P<String> const& inName,
                             size_t& outIndex, V& outGlobal) = 0;
  virtual int bindVar(Thread& th, P<String> const& inName,
                      size_t& outIndex) = 0;
  virtual int innerBindVar(Thread& th, P<String> const& inName,
                           size_t& outIndex);
};
...
class TopCompileScope : public CompileScope {
 public:
  ...
  virtual int directLookup(Thread& th, P<String> const& inName,
                           size_t& outIndex, V& outBuiltIn) override;
  virtual int indirectLookup(Thread& th, P<String> const& inName,
                             size_t& outIndex, V& outGlobal) override;
  virtual int bindVar(Thread& th, P<String> const& inName,
...
inline void RCObj::release() {
  ...
  if (newRefCount == 0) norefs();
  if (newRefCount < 0) negrefcount();
}
...
class SaveStack {
 ...
 public:
  SaveStack(Thread& _th, int n = 0) : th(_th), saveBase(th.stackBase) {
    th.setStackBase(n);
  }
  ~SaveStack() {
    th.clearStack();
...
class ParenStack {
 ...
 public:
  ParenStack(Thread& _th) : th(_th), saveBase(th.stackBase) {
    th.setStackBase();
  }
  ~ParenStack() { th.setStackBaseTo(saveBase); }
};
...
class UseRate {
 ...
 public:
  UseRate(Thread& _th, Rate const& inRate) : th(_th), prevRate(th.rate) {
    th.rate = inRate;
  }
  ~UseRate() { th.rate = prevRate; }
};

inline void Gen::setDone() { mDone = true; }
