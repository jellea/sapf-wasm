...
class DelayN : public Gen {
 ...
 public:
  DelayN(Thread& th, Arg in, Arg delay, Z maxdelay)
      : Gen(th, itemTypeZ, false), in_(in), delay_(delay), maxdelay_(maxdelay) {
...
class DelayL : public Gen {
 ...
 public:
  DelayL(Thread& th, Arg in, Arg delay, Z maxdelay)
      : Gen(th, itemTypeZ, false), in_(in), delay_(delay), maxdelay_(maxdelay) {
    sr = th.rate.sampleRate;
    bufSize = NEXTPOWEROFTWO((int32_t)ceil(sr * maxdelay));
    bufMask = bufSize - 1;
...
class DelayC : public Gen {
 ...
 public:
  DelayC(Thread& th, Arg in, Arg delay, Z maxdelay)
      : Gen(th, itemTypeZ, false), in_(in), delay_(delay), maxdelay_(maxdelay) {
    sr = th.rate.sampleRate;
    bufSize = NEXTPOWEROFTWO((int32_t)ceil(sr * maxdelay));
    bufMask = bufSize - 1;
...
class Flange : public Gen {
 ...
 public:
  Flange(Thread& th, Arg in, Arg delay, Z maxdelay)
      : Gen(th, itemTypeZ, false), in_(in), delay_(delay), maxdelay_(maxdelay) {
    sr = th.rate.sampleRate;
...
class Flangep : public Gen {
 ...
 public:
  Flangep(Thread& th, Arg in, Arg delay, Z maxdelay)
      : Gen(th, itemTypeZ, false), in_(in), delay_(delay), maxdelay_(maxdelay) {
    sr = th.rate.sampleRate;
    fhalf = ceil(sr * maxdelay + .5);
    half = (int32_t)fhalf;
...
class CombN : public Gen {
 ...
 public:
  CombN(Thread& th, Arg in, Arg delay, Z maxdelay, Arg decay)
      ...
      if (in_(th, n, inStride, in) || delay_(th, n, delayStride, delay) ||
        ...
        if (decayStride == 0) {
          ...
          for (int i = 0; i < n; ++i) {
            ...
            int32_t offset =
                std::max(1, (int32_t)floor(std::abs(zdelay) * sr + .5));
...
class CombL : public Gen {
 ...
 public:
  CombL(Thread& th, Arg in, Arg delay, Z maxdelay, Arg decay)
      : Gen(th, itemTypeZ, false),
        in_(in),
        delay_(delay),
        decay_(decay),
...
class CombC : public Gen {
 ...
 public:
  CombC(Thread& th, Arg in, Arg delay, Z maxdelay, Arg decay)
      : Gen(th, itemTypeZ, false),
        in_(in),
        delay_(delay),
        decay_(decay),
...
class LPCombC : public Gen {
 ...
 public:
  LPCombC(Thread& th, Arg in, Arg delay, Z maxdelay, Arg decay, Arg lpfreq)
      : Gen(th, itemTypeZ, false),
        in_(in),
        delay_(delay),
        decay_(decay),
        ...
        maxdelay_(maxdelay),
        y1_(0.),
        freqmul_(th.rate.invNyquistRate * kFirstOrderCoeffScale) {
    sr = th.rate.sampleRate;
    bufSize = NEXTPOWEROFTWO((int32_t)ceil(sr * maxdelay));
    bufMask = bufSize - 1;
    bufPos = 0;
  ...
  virtual void pull(Thread& th) override {
    ...
    while (framesToFill) {
      ...
      if (in_(th, n, inStride, in) || delay_(th, n, delayStride, delay) ||
          decay_(th, n, decayStride, decay) ||
          lpfreq_(th, n, lpfreqStride, lpfreq)) {
        setDone();
        break;
      } else {
        if (lpfreqStride == 0) {
...
class AllpassN : public Gen {
 ...
 public:
  AllpassN(Thread& th, Arg in, Arg delay, Z maxdelay, Arg decay)
      : Gen(th, itemTypeZ, false),
        in_(in),
        delay_(delay),
        decay_(decay),
...
class AllpassL : public Gen {
 ...
 public:
  AllpassL(Thread& th, Arg in, Arg delay, Z maxdelay, Arg decay)
      : Gen(th, itemTypeZ, false),
        in_(in),
        delay_(delay),
        decay_(decay),
...
class AllpassC : public Gen {
 ...
 public:
  AllpassC(Thread& th, Arg in, Arg delay, Z maxdelay, Arg decay)
      : Gen(th, itemTypeZ, false),
        in_(in),
        delay_(delay),
        decay_(decay),
...
class FDN : public Object {
 ...
 public:
  ...
  FDN(Thread& th, Arg in, Arg wet, Z mindelay, Z maxdelay, Z decayLo,
      Z decayMid, Z decayHi, Z seed)
      : in_(in),
        wet_(wet),
        decayLo_(decayLo),
        decayMid_(decayMid),
...
void FDN_OutputChannel::pull(Thread& th) { mFDN->pull(th); }
...
static void fdn_(Thread& th, Prim* prim) {
  ...
  P<FDN> fdn = new FDN(th, in, wet, mindelay, maxdelay, decayLo, decayMid,
                       decayHi, seed);
...
void AddDelayUGenOps() {
  ...
  DEFAM(delayn, zzk,
        "(in delay maxdelay --> out) delay line with no interpolation.");
  DEFAM(delayl, zzk,
        "(in delay maxdelay --> out) delay line with linear interpolation.");
  DEFAM(delayc, zzk,
        "(in delay maxdelay --> out) delay line with cubic interpolation.");
  DEFAM(flange, zzk,
        "(in delay maxdelay --> out) flanger with cubic interpolation. delay "
        "can be negative. latency is maxdelay.");
  DEFAM(flangep, zzk,
        "(in delay maxdelay --> out) flanger with cubic interpolation. adds "
        "delayed signal instead of subtracts.");

  DEFAM(combn, zzkz,
        "(in delay maxdelay decayTime --> out) comb delay filter with no "
        "interpolation.");
  DEFAM(combl, zzkz,
        "(in delay maxdelay decayTime --> out) comb delay filter with linear "
        "interpolation.");
  DEFAM(combc, zzkz,
        "(in delay maxdelay decayTime --> out) comb delay filter with cubic "
        "interpolation.");
  DEFAM(lpcombc, zzkzz,
        "(in delay maxdelay decayTime lpfreq --> out) low pass comb delay "
        "filter with cubic interpolation.");

  DEFAM(alpasn, zzkz,
        "(in delay maxdelay decayTime --> out) all pass delay filter with no "
        "interpolation.");
  DEFAM(alpasl, zzkz,
        "(in delay maxdelay decayTime --> out) all pass delay filter with "
        "linear interpolation.");
  DEFAM(alpasc, zzkz,
        "(in delay maxdelay decayTime --> out) all pass delay filter with "
        "cubic interpolation.");
  DEFAM(fdn, zzkkkkkk,
        "(in wet decayLo decayMid decayHi mindelay maxdelay rseed --> out) "
