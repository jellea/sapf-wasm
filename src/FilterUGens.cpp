...
struct Lag : public Gen {
  ...
  Lag(Thread& th, Arg in, Arg lagTime)
      : Gen(th, itemTypeZ, mostFinite(in, lagTime)),
        _in(in),
        _lagTime(lagTime),
        _y1(0.),
        _lagmul(log001 * th.rate.invSampleRate),
        once(true) {}
  ...
  virtual void pull(Thread& th) override {
    ...
    while (framesToFill) {
      ...
      if (lagTimeStride == 0) {
        Z b1 = std::max(
            0.,
            1. + lagmul / *lagTime);  // this is an approximation to exp(log001
                                      // * th.rate.invSampleRate / *lagTime)
        for (int i = 0; i < n; ++i) {
          Z y0 = *in;
        ...
        for (int i = 0; i < n; ++i) {
          Z b1 = std::max(
              0., 1. + lagmul /
                           *lagTime);  // this is an approximation to exp(log001
                                       // * th.rate.invSampleRate / *lagTime)
          Z y0 = *in;
          out[i] = y1 = y0 + b1 * (y1 - y0);
          in += inStride;
...
struct LagUD : public Gen {
  ...
  LagUD(Thread& th, Arg in, Arg riseTime, Arg fallTime)
      : Gen(th, itemTypeZ, mostFinite(in, riseTime, fallTime)),
        _in(in),
        _riseTime(riseTime),
        _fallTime(fallTime),
        _y1(0.),
  ...
  virtual void pull(Thread& th) override {
    ...
    while (framesToFill) {
      ...
      if (_in(th, n, inStride, in) ||
          _riseTime(th, n, riseTimeStride, riseTime) ||
          _fallTime(th, n, fallTimeStride, fallTime)) {
        setDone();
        break;
      }
      ...
      if (riseTimeStride == 0 && fallTimeStride == 0) {
        Z b1r = std::max(
            0.,
            1. + lagmul / *riseTime);  // this is an approximation to exp(log001
                                       // * th.rate.invSampleRate / *riseTime)
      ...
      if (riseTimeStride == 0 && fallTimeStride == 0) {
        ...
        for (int i = 0; i < n; ++i) {
          Z b1r = std::max(
              0.,
              1. + lagmul /
                       *riseTime);  // this is an approximation to exp(log001 *
                                    // th.rate.invSampleRate / *riseTime)
          Z b1f = std::max(0., 1. + lagmul / *fallTime);
...
struct FirstOrderLPF : public Gen {
  ...
  FirstOrderLPF(Thread& th, Arg in, Arg freq)
      : Gen(th, itemTypeZ, mostFinite(in, freq)),
        _in(in),
        _freq(freq),
        _x1(0.),
        _y1(0.),
        _freqmul(th.rate.invNyquistRate * kFirstOrderCoeffScale) {}
...
struct LPF : public Gen {
  ...
  LPF(Thread& th, Arg in, Arg freq)
      : Gen(th, itemTypeZ, mostFinite(in, freq)),
        ...
        _y1(0.),
        _y2(0.),
        _freqmul(th.rate.radiansPerSample * gInvSineTableOmega),
        _alphamul(.5 * M_SQRT2) {}
...
struct RLPF : public Gen {
  ...
  RLPF(Thread& th, Arg in, Arg freq, Arg rq)
      : Gen(th, itemTypeZ, mostFinite(in, freq, rq)),
        _in(in),
        _freq(freq),
        _rq(rq),
        _x1(0.),
...
struct BPF : public Gen {
  ...
  BPF(Thread& th, Arg in, Arg freq, Arg bw)
      : Gen(th, itemTypeZ, mostFinite(in, freq, bw)),
        _in(in),
        _freq(freq),
        _bw(bw),
        _x1(0.),
  ...
  virtual void pull(Thread& th) override {
    ...
    while (framesToFill) {
      ...
      for (int i = 0; i < n; ++i) {
        ...
        // bw * log(2) is the first term of the taylor series for
        // 2*sinh(log(2)/2 * bw) == 1/Q. the log(2) is combined with the .5 term
...
struct PEQ : public Gen {
  ...
  PEQ(Thread& th, Arg in, Arg freq, Arg bw, Arg gain)
      : Gen(th, itemTypeZ, mostFinite(in, freq, bw, gain)),
        _in(in),
        _freq(freq),
        _bw(bw),
        _gain(gain),
  ...
  virtual void pull(Thread& th) override {
    ...
    while (framesToFill) {
      ...
      n = framesToFill;
      if (_in(th, n, inStride, in) || _freq(th, n, freqStride, freq) ||
          _bw(th, n, bwStride, bw) || _gain(th, n, gainStride, gain)) {
        setDone();
        break;
      }
...
struct LowShelf : public Gen {
  ...
  LowShelf(Thread& th, Arg in, Arg freq, Arg gain)
      : Gen(th, itemTypeZ, mostFinite(in, freq, gain)),
        _in(in),
        _freq(freq),
        _gain(gain),
        _x1(0.),
...
struct LowShelf1 : public Gen {
  ...
  LowShelf1(Thread& th, Arg in, Arg freq, Arg gain)
      : Gen(th, itemTypeZ, mostFinite(in, freq, gain)),
        ...
        _x1(0.),
        _y1(0.),
        _freqmul(th.rate.invSampleRate) {}
...
struct Ringz : public Gen {
  ...
  Ringz(Thread& th, Arg in, Arg freq, Arg ringTime)
      : Gen(th, itemTypeZ, mostFinite(in, freq, ringTime)),
        _in(in),
        _freq(freq),
        _ringTime(ringTime),
        _x1(0.),
  ...
  virtual void pull(Thread& th) override {
    ...
    while (framesToFill) {
      ...
      n = framesToFill;
      if (_in(th, n, inStride, in) || _freq(th, n, freqStride, freq) ||
          _ringTime(th, n, ringTimeStride, ringTime)) {
        setDone();
        break;
      }
...
struct Formlet : public Gen {
  ...
  virtual void pull(Thread& th) override {
    ...
    while (framesToFill) {
      ...
      n = framesToFill;
      if (_in(th, n, inStride, in) || _freq(th, n, freqStride, freq) ||
          _atkTime(th, n, atkTimeStride, atkTime) ||
          _dcyTime(th, n, dcyTimeStride, dcyTime)) {
        setDone();
        break;
      }
...
struct KlankFilter {
  KlankFilter(V f, V a, V r)
      : freq(f), amp(a), ringTime(r), x1(0.), x2(0.), y1(0.), y2(0.) {}
...
struct Klank : public Gen {
  ...
  virtual ~Klank() { delete[] inputBuffer; }
  ...
  virtual void pull(Thread& th) override {
    ...
    for (size_t filter = 0; filter < _filters.size(); ++filter) {
      ...
      while (framesToFill) {
        ...
        n = framesToFill;
        if (kf.freq(th, n, freqStride, freq) || kf.amp(th, n, ampStride, amp) ||
            kf.ringTime(th, n, ringTimeStride, ringTime)) {
          setDone();
...
struct LeakDC : public Gen {
  ...
  LeakDC(Thread& th, Arg in, Arg leak)
      : Gen(th, itemTypeZ, mostFinite(in, leak)),
        _in(in),
        _leak(leak),
        _x1(0.),
        _y1(0.) {}
...
struct Decay : public Gen {
  ...
  Decay(Thread& th, Arg in, Arg decayTime)
      : Gen(th, itemTypeZ, mostFinite(in, decayTime)),
        _in(in),
  ...
  virtual void pull(Thread& th) override {
    ...
    while (framesToFill) {
      ...
      n = framesToFill;
      if (_in(th, n, inStride, in) ||
          _decayTime(th, n, decayTimeStride, decayTime)) {
        setDone();
        break;
      }
      ...
      if (decayTimeStride == 0) {
        Z b1 = std::max(
            0., 1. + lagmul /
                         *decayTime);  // this is an approximation to exp(log001
                                       // * th.rate.invSampleRate / *lagTime)
        for (int i = 0; i < n; ++i) {
          Z x0 = *in;
          out[i] = y1 = x0 + b1 * y1;
...
struct Decay2 : public Gen {
  ...
  Decay2(Thread& th, Arg in, Arg attackTime, Arg decayTime)
      : Gen(th, itemTypeZ, mostFinite(in, attackTime, decayTime)),
        _in(in),
  ...
  virtual void pull(Thread& th) override {
    ...
    while (framesToFill) {
      ...
      n = framesToFill;
      if (_in(th, n, inStride, in) ||
          _attackTime(th, n, attackTimeStride, attackTime) ||
          _decayTime(th, n, decayTimeStride, decayTime)) {
        setDone();
        break;
      }
      ...
      if (attackTimeStride == 0 && decayTimeStride == 0) {
        Z b1a = std::max(
            0.,
            1. + lagmul /
                     *attackTime);  // this is an approximation to exp(log001 *
                                    // th.rate.invSampleRate / *lagTime)
        Z b1b = std::max(
            0., 1. + lagmul /
                         *decayTime);  // this is an approximation to exp(log001
  ...
  AmpFollow(Thread& th, Arg in, Z atk, Z dcy)
    ...
    b1r_ =
        atk == 0.
            ...
            : std::max(0.,
                       1. + _lagmul /
                                atk);  // this is an approximation to exp(log001
                                       // * th.rate.invSampleRate / *riseTime)
...
void AddFilterUGenOps() {
  ...
  DEFMCX(lag, 2,
         "(in decayTime --> out) one pole lag filter. decayTime determines "
         "rate of convergence.")
  DEFMCX(lag2, 2,
         "(in decayTime --> out) cascade of two one pole lag filters. "
         "decayTime determines rate of convergence.")
  DEFMCX(lag3, 2,
         "(in decayTime --> out) cascade of three one pole lag filters. "
         "decayTime determines rate of convergence.")

  DEFMCX(lagud, 3,
         "(in upDecayTime, downDecayTime --> out) one pole lag filter. up/down "
         "DecayTimes determines rate of convergence up/down.")
  DEFMCX(lagud2, 3,
         "(in upDecayTime, downDecayTime --> out) cascade of two one pole lag "
         "filters. up/down DecayTimes determines rate of convergence up/down.")
  DEFMCX(
      lagud3, 3,
      "(in upDecayTime, downDecayTime --> out) cascade of three one pole lag "
      "filters. up/down DecayTimes determines rate of convergence up/down.")
  ...
  DEFMCX(rlpf, 3,
         "(in freq rq --> out) resonant low pass filter. 12 dB/oct slope. rq "
         "is 1/Q.")
  DEFMCX(rhpf, 3,
         "(in freq rq --> out) resonant high pass filter. 12 dB/oct slope. rq "
         "is 1/Q.")
  DEFMCX(rlpf2, 3,
         "(in freq rq --> out) resonant low pass filter. 24 dB/oct slope. rq "
         "is 1/Q.")
  DEFMCX(rhpf2, 3,
         "(in freq rq --> out) resonant high pass filter. 24 dB/oct slope. rq "
         "is 1/Q.")

  DEFMCX(rlpfc, 3,
         "(in freq rq --> out) resonant low pass filter with saturation. 12 "
         "dB/oct slope. rq is 1/Q.")
  DEFMCX(rhpfc, 3,
         "(in freq rq --> out) resonant high pass filter with saturation. 12 "
         "dB/oct slope. rq is 1/Q.")
  DEFMCX(rlpf2c, 3,
         "(in freq rq --> out) resonant low pass filter with saturation. 24 "
         "dB/oct slope. rq is 1/Q.")
  DEFMCX(rhpf2c, 3,
         "(in freq rq --> out) resonant high pass filter with saturation. 24 "
         "dB/oct slope. rq is 1/Q.")

  DEFMCX(bpf, 3,
         "(in freq bw --> out) band pass filter. bw is bandwidth in octaves.")
  DEFMCX(bsf, 3,
         "(in freq bw --> out) band stop filter. bw is bandwidth in octaves.")
  DEFMCX(apf, 3,
         "(in freq bw --> out) all pass filter. bw is bandwidth in octaves.")

  DEFMCX(peq, 4,
         "(in freq bw gain --> out) parametric equalization filter. bw is "
  ...
  DEFMCX(ringz, 3,
         "(in freq ringTime --> out) resonant filter specified by a ring time "
         "in seconds.")
  DEFMCX(formlet, 4,
         "(in freq atkTime dcyTime --> out) a formant filter whose impulse "
         "response is a sine grain.")
  DEFAM(klank, zaaa,
        "(in freqs amps ringTimes --> out) a bank of ringz filters. freqs amps "
        "and ringTimes are arrays.")

  DEFMCX(leakdc, 2, "(in coef --> out) leaks away energy at 0 Hz.")
  DEFMCX(leaky, 2, "(in coef --> out) leaky integrator.")
  DEFMCX(decay, 2,
         "(in decayTime --> out) outputs an exponential decay for impulses at "
         "the input.")
  DEFMCX(decay2, 3,
         "(in atkTime dcyTime --> out) outputs an exponential attack and decay "
         "for impulses at the input.")

  DEFMCX(hilbert, 1,
         "(in --> outA outB) returns two signals that are 90 degrees phase "
