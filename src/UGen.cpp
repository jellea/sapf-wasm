...
struct MulAdd : public ThreeInputUGen<MulAdd> {
  MulAdd(Thread& th, Arg a, Arg b, Arg c)
      : ThreeInputUGen<MulAdd>(th, a, b, c) {}

  virtual const char* TypeName() const override { return "MulAdd"; }

  void calc(int n, Z* out, Z* a, Z* b, Z* c, int aStride, int bStride,
            int cStride) {
    for (int i = 0; i < n; ++i) {
      out[i] = *a * *b + *c;
      a += aStride;
  ...
  if (a.isReal() && b.isReal() && c.isReal()) {
    ...
    if (c.isReal()) {
      if (c.f == 0.) {
        ...
        if (a.isReal() && a.f == -1.) {
          th.push(b.unaryOp(th, gUnaryOpPtr_neg));
          return;
        }
        if (b.isReal() && b.f == -1.) {
          th.push(a.unaryOp(th, gUnaryOpPtr_neg));
          return;
        }
        th.push(a.binaryOp(th, gBinaryOpPtr_mul, b));
        return;
      }
    }
    if (a.isReal()) {
      ...
      if (a.f == 1.) {
        th.push(b.binaryOp(th, gBinaryOpPtr_plus, c));
        return;
      }
      if (a.f == -1.) {
        th.push(b.binaryOp(th, gBinaryOpPtr_minus, c));
        return;
      }
    }
    if (b.isReal()) {
      ...
      if (b.f == 1.) {
        th.push(a.binaryOp(th, gBinaryOpPtr_plus, c));
        return;
      }
      if (b.f == -1.) {
        th.push(a.binaryOp(th, gBinaryOpPtr_minus, c));
        return;
      }
    }
...
struct Fadeout : Gen {
  ...
  Fadeout(Thread& th, Arg a, Z sustainTime, Z fadeTime)
      : Gen(th, itemTypeZ, true), _a(a) {
...
struct Endfade : Gen {
  ...
  Endfade(Thread& th, Arg a, Z startupTime, Z holdTime, Z fadeTime, Z threshold)
      : Gen(th, itemTypeZ, true), _a(a) {
...
struct Imps : public Gen {
  ...
  Imps(Thread& th, Arg durs, Arg vals, Arg rate)
      : Gen(th, itemTypeZ, mostFinite(vals, durs, rate)),
        durs_(durs),
        vals_(vals),
        rate_(rate),
        phase_(0.),
...
struct Steps : public Gen {
  ...
  Steps(Thread& th, Arg durs, Arg vals, Arg rate)
      : Gen(th, itemTypeZ, mostFinite(vals, durs, rate)),
        durs_(durs),
        vals_(vals),
        rate_(rate),
...
struct Gates : public Gen {
  ...
  Gates(Thread& th, Arg durs, Arg vals, Arg hold, Arg rate)
      : Gen(th, itemTypeZ, mostFinite(vals, durs, rate, hold)),
        durs_(durs),
        vals_(vals),
        hold_(hold),
        rate_(rate),
...
struct Lines : public Gen {
  ...
  Lines(Thread& th, Arg durs, Arg vals, Arg rate)
      : Gen(th, itemTypeZ, mostFinite(vals, durs, rate)),
        durs_(durs),
        vals_(vals),
        rate_(rate),
...
struct XLines : public Gen {
  ...
  XLines(Thread& th, Arg durs, Arg vals, Arg rate)
      : Gen(th, itemTypeZ, mostFinite(vals, durs, rate)),
        durs_(durs),
        vals_(vals),
        rate_(rate),
...
struct Curves : public Gen {
  ...
  Curves(Thread& th, Arg durs, Arg vals, Arg curves, Arg rate)
      : Gen(th, itemTypeZ, mostFinite(vals, durs, rate, curves)),
        durs_(durs),
        vals_(vals),
        curves_(curves),
        rate_(rate),
      ...
      if (rateStride == 0) {
        for (int i = 0; i < n; ++i) {
          while (phase_ >= dur_) {
            ...
            do {
              oldval_ = newval_;
              if (vals_.onez(th, newval_) || curves_.onez(th, curve_) ||
                  durs_.onez(th, dur_)) {
                setDone();
                goto leave;
              }
...
struct Cubics : public Gen {
  ...
  Cubics(Thread& th, Arg vals, Arg rate)
      : Gen(th, itemTypeZ, mostFinite(vals, rate)),
        vals_(vals),
        rate_(rate),
        phase_(1.),
        freqmul_(th.rate.invSampleRate) {}
...
struct Tempo : public Gen {
  ...
  Tempo(Thread& th, Arg vals, Arg rate)
      : Gen(th, itemTypeZ, mostFinite(vals, rate)),
        vals_(vals),
        rate_(rate),
  ...
  virtual void pull(Thread& th) override {
    ...
    while (framesToFill) {
      ...
      for (int i = 0; i < n; ++i) {
        // numerically it would be better to subtract off dur each time, but we
        // need to recreate the same loss of precision over time as will be
        // experienced from an integration of tempo occuring outside of this
        // generator. otherwise there would be a drift between when tempo
        // changes occur and the beat time as integrated from the tempo.
...
struct Beats : public Gen {
  ...
  Beats(Thread& th, Arg tempo)
      : Gen(th, itemTypeZ, tempo.isFinite()),
        tempo_(tempo),
        invsr_(th.rate.invSampleRate) {}
...
struct ADSR : public Gen {
  ...
  ADSR(Thread& th, Z* levels, Z* durs, Z* curves, Arg rate, int sustainStage)
      : Gen(th, itemTypeZ, true),
        rate_(rate),
        sustainStage_(sustainStage),
        freqmul_(th.rate.invSampleRate) {
  ...
  virtual void pull(Thread& th) override {
    ...
    while (framesToFill) {
      ...
      //
      //	limit n to the min(blockSize, stageSamplesRemaining,
      //[noteOffSample]) 	after a stage
...
struct GatedADSR : public Gen {
  ...
  GatedADSR(Thread& th, Z* levels, Z* durs, Z* curves, Arg gate,
            int sustainStage)
      : Gen(th, itemTypeZ, true),
        gate_(gate),
        sustainStage_(sustainStage),
...
static void adsr_(Thread& th, Prim* prim) {
  ...
  th.push(
      new List(new ADSR<kNumADSRStages>(th, levels, durs, curves, rate, 2)));
}
...
static void dadsr_(Thread& th, Prim* prim) {
  ...
  if (list->fillz(th, kNumADSRStages, env) != kNumADSRStages) {
    post(
        "dahdsr : [delay attack decay sustain release] list should have 5 "
        "elements.");
  }
  ...
  th.push(
      new List(new ADSR<kNumADSRStages>(th, levels, durs, curves, rate, 3)));
}
...
static void dahdsr_(Thread& th, Prim* prim) {
  ...
  P<List> list =
      th.popList("dahdsr : [delay attack hold decay sustain release]");

  const int kNumADSRStages = 6;
  Z env[kNumADSRStages];
  if (list->fillz(th, kNumADSRStages, env) != kNumADSRStages) {
    post(
        "dahdsr : [delay attack hold decay sustain release] list should have 6 "
        "elements.");
  }
  ...
  Z levels[kNumADSRStages + 1] = {0.,           0.,           amp, amp,
                                  amp * susLvl, amp * susLvl, 0.};
  Z durs[kNumADSRStages] = {dlyTime, atkTime, hldTime,
                            dcyTime, noteDur, relTime};
  Z curves[kNumADSRStages] = {0., -1., 0., -5., 0., -5.};

  th.push(
      new List(new ADSR<kNumADSRStages>(th, levels, durs, curves, rate, 4)));
}
...
struct K2A : public Gen {
  ...
  K2A(Thread& th, int n, Arg vals)
      : Gen(th, itemTypeZ, vals.isFinite()),
        vals_(vals),
        n_(n),
        remain_(0),
        slopeFactor_(1. / n) {}
    ...
    while (framesToFill) {
      ...
      for (int i = 0; i < n; ++i) {
        while (x >= 1.) {
          ...
          y2 = y3;
          y3 = r.drand2() *
               0.8;  // 0.8 because cubic interpolation can overshoot up to 1.25
                     // if inputs are -1,1,1,-1.
          c0 = y1;
          c1 = .5 * (y2 - y0);
          c2 = y0 - 2.5 * y1 + 2. * y2 - .5 * y3;
          c3 = 1.5 * (y1 - y2) + .5 * (y3 - y0);
...
struct SymmetricEnv : public Gen {
  ...
  SymmetricEnv<F>(Thread& th, Z dur, Z scale)
      : Gen(th, itemTypeZ, true), x(-scale) {
...
struct TParEnv : public TriggeredSymmetricEnv<TParEnv> {
  TParEnv(Thread& th, Arg trig, Arg dur, Arg amp)
      : TriggeredSymmetricEnv<TParEnv>(th, trig, dur, amp, 1.) {}
...
struct TQuadEnv : public TriggeredSymmetricEnv<TQuadEnv> {
  TQuadEnv(Thread& th, Arg trig, Arg dur, Arg amp)
      : TriggeredSymmetricEnv<TQuadEnv>(th, trig, dur, amp, 1.) {}
...
struct TOctEnv : public TriggeredSymmetricEnv<TOctEnv> {
  TOctEnv(Thread& th, Arg trig, Arg dur, Arg amp)
      : TriggeredSymmetricEnv<TOctEnv>(th, trig, dur, amp, 1.) {}
...
struct TTriEnv : public TriggeredSymmetricEnv<TTriEnv> {
  TTriEnv(Thread& th, Arg trig, Arg dur, Arg amp)
      : TriggeredSymmetricEnv<TTriEnv>(th, trig, dur, amp, 1.) {}
...
struct TTri2Env : public TriggeredSymmetricEnv<TTri2Env> {
  TTri2Env(Thread& th, Arg trig, Arg dur, Arg amp)
      : TriggeredSymmetricEnv<TTri2Env>(th, trig, dur, amp, 1.) {}
...
struct TTrapezEnv : public TriggeredSymmetricEnv<TTrapezEnv> {
  TTrapezEnv(Thread& th, Arg trig, Arg dur, Arg amp)
      : TriggeredSymmetricEnv<TTrapezEnv>(th, trig, dur, amp, 1.) {}
...
struct TTrapez2Env : public TriggeredSymmetricEnv<TTrapez2Env> {
  TTrapez2Env(Thread& th, Arg trig, Arg dur, Arg amp)
      : TriggeredSymmetricEnv<TTrapez2Env>(th, trig, dur, amp, 1.) {}
...
struct TCosEnv : public TriggeredSymmetricEnv<TCosEnv> {
  TCosEnv(Thread& th, Arg trig, Arg dur, Arg amp)
      : TriggeredSymmetricEnv<TCosEnv>(th, trig, dur, amp, 1.) {}
...
struct THanEnv : public TriggeredSymmetricEnv<THanEnv> {
  THanEnv(Thread& th, Arg trig, Arg dur, Arg amp)
      : TriggeredSymmetricEnv<THanEnv>(th, trig, dur, amp, 1.) {}
...
struct THan2Env : public TriggeredSymmetricEnv<THan2Env> {
  THan2Env(Thread& th, Arg trig, Arg dur, Arg amp)
      : TriggeredSymmetricEnv<THan2Env>(th, trig, dur, amp, 1.) {}
...
struct GaussEnv : public SymmetricEnv<GaussEnv> {
  Z widthFactor;
  GaussEnv(Thread& th, Z dur, Z width)
      : SymmetricEnv<GaussEnv>(th, dur, 1.),
        widthFactor(-1. / (2. * width * width)) {}
...
class OverlapAdd : public OverlapAddBase {
 ...
 public:
  OverlapAdd(Thread& th, Arg sounds, Arg hops, Arg rate,
             P<Form> const& chasedSignals, int numChannels);
  virtual ~OverlapAdd() {}
...
class OverlapAddInputSource : public Object {
 public:
  ...
  OverlapAddInputSource(Thread& th, List* channels, int inOffset,
                        P<OverlapAddInputSource> const& inNextSource)
...
class OverlapAddOutputChannel : public Gen {
 ...
 public:
  ...
  virtual const char* TypeName() const override {
    return "OverlapAddOutputChannel";
  }
...
OverlapAddBase::OverlapAddBase(int numChannels) : mNumChannels(numChannels) {}
...
OverlapAdd::OverlapAdd(Thread& th, Arg sounds, Arg hops, Arg rate,
                       P<Form> const& chasedSignals, int numChannels)
    : OverlapAddBase(numChannels),
      ...
      mRate(rate),
      mBeatTime(0.),
      mNextEventBeatTime(0.),
      mEventCounter(0.),
      mRateMul(th.rate.invSampleRate),
      mSampleTime(0),
      mPrevChaseTime(0),
    ...
    for (int i = 0; i < blockSize; ++i) {
      while (beatTime >= nextEventBeatTime) {
        ...
        if (out.isZList() || (out.isVList() && out.isFinite())) {
          ...
          P<OverlapAddInputSource> source =
              new OverlapAddInputSource(th, s, i, mActiveSources);
...
int OverlapAddBase::renderActiveSources(Thread& th, int blockSize,
                                        bool& anyDone) {
  int maxProduced = 0;
  ...
  while (source) {
    ...
    OverlapAddOutputChannel* output = mOutputs;
    for (size_t j = 0; j < sourceChannels.size() && output;
         ++j, output = output->mNextOutput) {
      if (output->mOut) {
        ZIn& zin = sourceChannels[j];
        if (zin.mIsConstant && zin.mConstant.f == 0.) continue;
...
void OverlapAddBase::produceOutputs(int shrinkBy) {
  OverlapAddOutputChannel* output = mOutputs;
  do {
    if (output->mOut) output->produce(shrinkBy);
    output = output->mNextOutput;
  } while (output);
}
...
bool OverlapAddBase::pull(Thread& th) {
  ...
  if (anyDone) removeInactiveSources();
...
static void ola_(Thread& th, Prim* prim) {
  ...
  P<OverlapAdd> ola =
      new OverlapAdd(th, sounds, hops, rate, chasedSignals, (int)numChannels);
...
struct ITD : public Gen {
  ...
  ITD(Thread& th, Arg in, Arg pan, Z maxdelay)
      : Gen(th, itemTypeZ, false), in_(in), pan_(pan), maxdelay_(maxdelay) {
...
ITD_OutputChannel::ITD_OutputChannel(Thread& th, bool inFinite, ITD* inITD)
    : Gen(th, itemTypeZ, inFinite), mITD(inITD) {}

void ITD_OutputChannel::pull(Thread& th) { mITD->pull(th); }
...
inline Z fast_cos1(Z x) { return fast_sin1(x + .25); }
...
struct Pan2 : public Object {
  ...
  Pan2(Thread& th, Arg inIn, Arg inPos) : _in(inIn), _pos(inPos) {
...
struct Pan2Out : public Gen {
  ...
  Pan2Out(Thread& th, bool inFinite, P<Pan2> const& inPan2)
      : Gen(th, itemTypeZ, inFinite), mPan2(inPan2) {}
  ...
  virtual void pull(Thread& th) override { mPan2->pull(th); }
};
...
struct Balance2Out : public Gen {
  ...
  Balance2Out(Thread& th, bool inFinite, P<Balance2> const& inBalance2)
      : Gen(th, itemTypeZ, inFinite), mBalance2(inBalance2) {}
  ...
  virtual void pull(Thread& th) override { mBalance2->pull(th); }
};
...
void Balance2::pull(Thread& th) {
  ...
  while (framesToFill) {
    ...
    n = framesToFill;
    if (_L(th, n, aStride, a) || _R(th, n, bStride, b) ||
        _pos(th, n, cStride, c)) {
      mLeft->setDone();
      mRight->setDone();
      break;
...
struct Rot2Out : public Gen {
  ...
  Rot2Out(Thread& th, bool inFinite, P<Rot2> const& inRot2)
      : Gen(th, itemTypeZ, inFinite), mRot2(inRot2) {}
  ...
  virtual void pull(Thread& th) override { mRot2->pull(th); }
};
...
struct Fade2 : public Gen {
  ...
  Fade2(Thread& th, Arg inL, Arg inR, Arg inPos)
      : Gen(th, itemTypeZ, mostFinite(inL, inR, inPos)),
        _L(inL),
        _R(inR),
        _pos(inPos) {}
...
struct Gate : public TwoInputUGen<Gate> {
  ...
  Gate(Thread& th, Arg trig, Arg hold)
      : TwoInputUGen<Gate>(th, trig, hold),
        phase(INFINITY),
        freq(th.rate.invSampleRate) {}
...
struct PulseDivider : public Gen {
  ...
  PulseDivider(Thread& th, Arg tr, Arg div, Z start)
      : Gen(th, itemTypeZ, mostFinite(tr, div)),
        _tr(tr),
        _div(div),
        _count(start - 1.) {}
...
static void* gstate_update_func(void* arg) {
  ...
  CGDirectDisplayID display =
      kCGDirectMainDisplay;  // to grab the main display ID
  ...
  for (;;) {
    ...
    gstate->mouseY =
        1. - point.y * rscreenHeight;  //(float)p.v * rscreenHeight;
...
struct MouseX : public TwoInputUGen<MouseX> {
  ...
  MouseX(Thread& th, Arg lo, Arg hi)
      : TwoInputUGen<MouseX>(th, lo, hi),
        _b1(1. + gMouseLagMul * th.rate.invSampleRate),
        _once(true) {}
...
struct MouseY : public TwoInputUGen<MouseY> {
  ...
  MouseY(Thread& th, Arg lo, Arg hi)
      : TwoInputUGen<MouseY>(th, lo, hi),
        _b1(1. + gMouseLagMul * th.rate.invSampleRate),
        _once(true) {}
...
struct ExpMouseX : public TwoInputUGen<ExpMouseX> {
  ...
  ExpMouseX(Thread& th, Arg lo, Arg hi)
      : TwoInputUGen<ExpMouseX>(th, lo, hi),
        _b1(1. + gMouseLagMul * th.rate.invSampleRate),
        _once(true) {}
...
struct ExpMouseY : public TwoInputUGen<ExpMouseY> {
  ...
  ExpMouseY(Thread& th, Arg lo, Arg hi)
      : TwoInputUGen<ExpMouseY>(th, lo, hi),
        _b1(1. + gMouseLagMul * th.rate.invSampleRate),
        _once(true) {}
...
void AddUGenOps() {
  ...
  DEF(kr, 2,
      "(fun n --> out) evaluates fun with the current sample rate divided by"
      "n, then linearly upsamples all returned signals by n.")
  DEF(krc, 2,
      "(fun n --> out) evaluates fun with the current sample rate divided by "
      "n, then cubically upsamples all returned signals by n.")
  ...
  DEFAM(tempo, az,
        "([bps dur bps dur ...] rate --> out) returns a signal of tempo vs "
        "time given a list of interleaved tempos (in beats per second) and "
        "durations (in beats).");
  DEFAM(beats, z,
        "(tempo --> beats) integrates a tempo signal to produce a signal of "
        "the time in beats.");

  vm.addBifHelp("\n*** envelope unit generators ***");
  vm.addBifHelp(
      "\nFor asr, adsr, dadsr, dahdsr envelopes, the arguments are as "
      "follows:");
  vm.addBifHelp(
      "   delay - a time in seconds. a period of time before the attack "
      "segment where the amplitude is zero.");
  vm.addBifHelp(
      "   attack - a time in seconds to rise from zero to the level specified "
      "by the amp argument.");
  vm.addBifHelp(
      "   hold - a time in seconds to hold at the level specified by the amp "
      "argument.");
  vm.addBifHelp(
      "   delay - a time in seconds to fall from amp to the sustain level.");
  vm.addBifHelp(
      "   sustain - a level from zero to one which is multiplied by the amp "
      "argument. The envelope holds at this level until released.");
  vm.addBifHelp(
      "   release - a time in seconds to fall from the current level to zero. "
      "A release begins whenever the beat time (the integral of tempo), "
      "exceeds dur.");
  vm.addBifHelp(
      "   amp - an amplitude that scales the peak and sustain levels of the "
      "envelope.");
  ...
  DEFAM(adsr, akkz,
        "([attack decay sustain release] amp dur tempo --> envelope) an "
        "envelope generator.")
  DEFAM(dadsr, akkz,
        "([delay attack decay sustain release] amp dur tempo --> envelope) an "
        "envelope generator.")
  DEFAM(dahdsr, akkz,
        "([delay attack hold decay sustain release] amp dur tempo --> "
        "envelope) an envelope generator.")
  vm.addBifHelp("");

  DEFAM(endfade, zkkkk,
        "(in startupTime holdTime fadeTime threshold --> out) after "
        "startupTime has elapsed, fade out the sound when peak amplitude has "
        "dropped below threshold for more than the holdTime.");
  DEFAM(fadeout, zkk,
        "(in sustainTime fadeTime --> out) fadeout after sustain.");
  DEFAM(fadein, zk, "(in fadeTime --> out) fade in.");
  DEFAM(parenv, k, "(dur --> out) parabolic envelope. 1-x^2 for x from -1 to 1")
  DEFAM(quadenv, k,
        "(dur --> out) 4th order envelope. 1-x^4 for x from -1 to 1")
  DEFAM(octenv, k, "(dur --> out) 8th order envelope. 1-x^8 for x from -1 to 1")
  DEFAM(trienv, k,
        "(dur --> out) triangular envelope. 1-|x| for x from -1 to 1")
  DEFAM(tri2env, k,
        "(dur --> out) triangle squared envelope. (1-|x|)^2 for x from -1 to 1")
  DEFAM(trapezenv, k,
        "(dur --> out) trapezoidal envelope. (2 - |x-.5| - |x+.5|) for x from "
        "-1 to 1")
  DEFAM(trapez2env, k,
        "(dur --> out) trapezoid squared envelope. (2 - |x-.5| - |x+.5|)^2 for "
        "x from -1 to 1")
  ...
  DEFAM(gaussenv, kk,
        "(dur width --> out) gaussian envelope. exp(x^2/(-2*width^2)) for x "
        "from -1 to 1")

  DEFAM(tsig, zza, "(trig signal amp --> out) trigger a signal.")

  DEFAM(tparenv, zaa,
        "(trig dur amp --> out) triggered parabolic envelope. 1-x^2 for x from "
        "-1 to 1")
  DEFAM(tquadenv, zaa,
        "(trig dur amp --> out) triggered 4th order envelope. 1-x^4 for x from "
        "-1 to 1")
  DEFAM(toctenv, zaa,
        "(trig dur amp --> out) triggered 8th order envelope. 1-x^8 for x from "
        "-1 to 1")
  DEFAM(ttrienv, zaa,
        "(trig dur amp --> out) triggered triangular envelope. 1-|x| for x "
        "from -1 to 1")
  DEFAM(ttri2env, zaa,
        "(trig dur amp --> out) triggered triangle squared envelope. (1-|x|)^2 "
        "for x from -1 to 1")
  DEFAM(ttrapezenv, zaa,
        "(trig dur amp --> out) triggered trapezoidal envelope. (2 - |x-.5| - "
        "|x+.5|) for x from -1 to 1")
  DEFAM(ttrapez2env, zaa,
        "(trig dur amp --> out) triggered trapezoid squared envelope. (2 - "
        "|x-.5| - |x+.5|)^2 for x from -1 to 1")
  ...
  DEFAM(than2env, zaa,
        "(trig dur amp --> out) triggered hanning squared envelope.")

  vm.addBifHelp("\n*** spawn unit generators ***");
  DEF(ola, 4,
      "(sounds hops rate numChannels --> out) overlap add. This is the basic"
      "operator for polyphony. ")

  // vm.addBifHelp("\n*** pause unit generator ***");
  DEFMCX(pause, 2,
         "(in amp --> out) pauses the input when amp is <= 0, otherwise in is "
         "multiplied by amp.")

  vm.addBifHelp("\n*** panner unit generators ***");
  DEFAM(itd, zzk, "(in pan maxdelay --> out) interaural time delay.");
  DEFMCX(pan2, 2,
         "(in pos --> [left right]) stereo pan. pos 0 is center. pos -1 is "
         "full left, pos +1 is full right.")
  DEFMCX(
      rot2, 3,
      "(left right pos --> [left right]) stereo rotation. pos 0 is no "
      "rotation, +/-1 is 180 degrees, -.5 is -90 degrees, +.5 is +90 degrees.")
  DEFMCX(bal2, 3,
         "(left right pos --> [left right]) stereo balance control. pos 0 is "
         "center. pos -1 is full left, pos +1 is full right.")
  DEFMCX(fade2, 3,
         "(left right pos --> out) cross fade between two inputs. pos 0 is "
         "equal mix. pos -1 is all left, pos +1 is all right.")

  vm.addBifHelp("\n*** trigger unit generators ***");
  DEFMCX(tr, 1,
         "(in --> out) transitions from nonpositive to positive become single "
         "sample impulses.")
  DEFMCX(ntr, 1,
         "(in --> out) transitions from negative to nonnegative become single "
         "sample impulses.")
  DEFMCX(gate, 1,
         "(in hold --> out) outputs 1 for hold seconds after each trigger, "
         "else outputs zero.")
  DEFMCX(sah, 2, "(in trigger --> out) sample and hold")
  DEFAM(seq, az,
        "(in trigger --> out) pulls one value from the input for each trigger. "
        "output sustains at that level until the next trigger.")
  DEFAM(iseq, az,
        "(in trigger --> out) pulls one value from the input for eachtrigger. "
        "outputs that value for one sample. outputs zero when there is no"
        "trigger.")
  DEFMCX(pdiv, 3,
         "(in n istart --> out) pulse divider. outputs one impulse from the "
         "output for each n impulses in the input. istart is an offset. istart "
         "= 0 outputs a pulse on the first input pulse.")

  // vm.addBifHelp("\n*** bounds unit generators ***");
  DEFMCX(clip, 3,
         "(in lo hi --> out) constrain the input to the bounds by clipping.")
  DEFMCX(wrap, 3,
         "(in lo hi --> out) constrain the input to the bounds by wrapping.")
  DEFMCX(fold, 3,
         "(in lo hi --> out) constrain the input to the bounds by folding "
         "at the edges.")
  DEFMCX(iwrap, 3,
         "(in lo hi --> out) constrain the input to the bounds by wrapping. "
         "all inputs treated as integers.")
  DEFMCX(ifold, 3,
         "(in lo hi --> out) constrain the input to the bounds by folding at "
         "the edges. all inputs treated as integers.")

#ifdef SAPF_CARBON
  vm.addBifHelp("\n*** mouse control unit generators ***");
  DEFMCX(mousex, 2,
         "(lo hi --> out) returns a signal of the X coordinate of the mouse "
         "mapped to the linear range lo to hi.");
  DEFMCX(mousey, 2,
         "(lo hi --> out) returns a signal of the Y coordinate of the mouse "
         "mapped to the linear range lo to hi.");
  DEFMCX(xmousex, 2,
         "(lo hi --> out) returns a signal of the X coordinate of the mouse "
         "mapped to the exponential range lo to hi.");
  DEFMCX(xmousey, 2,
         "(lo hi --> out) returns a signal of the Y coordinate of the mouse "
         "mapped to the exponential range lo to hi.");

  DEFMCX(mousex1, 2,
         "(lo hi --> out) returns the current value of the X coordinate of the "
         "mouse mapped to the linear range lo to hi.");
  DEFMCX(mousey1, 2,
         "(lo hi --> out) returns the current value of the Y coordinate of the "
         "mouse mapped to the linear range lo to hi.");
  DEFMCX(xmousex1, 2,
         "(lo hi --> out) returns the current value of the X coordinate of the "
         "mouse mapped to the exponential range lo to hi.");
  DEFMCX(xmousey1, 2,
         "(lo hi --> out) returns the current value of the Y coordinate of the "
         "mouse mapped to the exponential range lo to hi.");
#else
