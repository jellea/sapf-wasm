...
static const char* prompt() { return "sapf> "; }
static const char* promptParen() { return "(sapf> "; }
static const char* promptSquareBracket() { return "[sapf> "; }
static const char* promptCurlyBracket() { return "{sapf> "; }
static const char* promptLambda() { return "\\sapf> "; }
static const char* promptString() { return "\"sapf> "; }

void compileSmth(Thread th, char* inString) {
  try {
    // PARSE
    {
      P<Fun> compiledFun;
...
void Thread::getLine() {
  ...
  switch (parsingWhat) {
    ...
    case parsingWords:
      ...
      break;
    case parsingString:
      el_set(el, EL_PROMPT, &promptString);
      break;
    case parsingParens:
      el_set(el, EL_PROMPT, &promptParen);
      break;
    case parsingLambda:
      el_set(el, EL_PROMPT, &promptLambda);
      break;
    case parsingArray:
      el_set(el, EL_PROMPT, &promptSquareBracket);
      break;
    case parsingEnvir:
      el_set(el, EL_PROMPT, &promptCurlyBracket);
      break;
  }
  line = el_gets(el, &linelen);
  linepos = 0;
  if (line == NULL || strncmp(line, "quit", 4) == 0 ||
      strncmp(line, "..", 2) == 0) {
    line = NULL;
    throw errUserQuit;
  }
  ...
  //     break;
  //   case parsingString:
  //     rl_set_prompt(promptString());
  //     break;
  //   case parsingParens:
  //     rl_set_prompt(promptParen());
  //     break;
  //   case parsingLambda:
  //     rl_set_prompt(promptLambda());
  //     break;
  //   case parsingArray:
  //     rl_set_prompt(promptSquareBracket());
  //     break;
  //   case parsingEnvir:
  //     rl_set_prompt(promptCurlyBracket());
  //     break;
  // }
  // line = rl_gets();
  // linelen = strlen(line);
  // linepos = 0;
  // if (strncmp(line, "quit", 4) == 0 || strncmp(line, "..", 2) == 0) {
  //   line = NULL;
  //   throw errUserQuit;
  // }
  // if (line && linelen) {
...
//   }
  ...
  // duplicate entries should be cleared by default (I think this is the same as
  // H_SETUNIQUE?)
#endif
...
V VM::def(const char* name, int takes, int leaves, PrimFun pf, const char* help,
          Arg value, bool setNoEach) {
  V aPrim = new Prim(pf, value, takes, leaves, name, help);
  def(name, aPrim);

  if (setNoEach) aPrim.SetNoEachOps();

  if (help) addBifHelp(name, aPrim.GetAutoMapMask(), help);

  return aPrim;
}

V VM::defmcx(const char* name, int numArgs, PrimFun pf, const char* help,
             Arg value) {
...
V VM::defautomap(const char* name, const char* mask, PrimFun pf,
                 const char* help, Arg value) {
...
int64_t Thread::popInt(const char* msg) {
  ...
  double f = v.f;
  int64_t i;
  if (f >= (double)LLONG_MAX)
    i = LLONG_MAX;
  else if (f <= (double)LLONG_MIN)
    i = LLONG_MIN;
  else
    i = (int64_t)f;
  return i;
...
void Thread::tuck(size_t n, Arg v) {
  ...
  for (size_t i = 0; i < n; ++i) sp[-i] = sp[-i - 1];
...
bool Thread::compile(const char* inString, P<Fun>& compiledFun,
                     bool inTopLevel) {
  Thread& th = *this;
  ...
  compiledFun =
      new Fun(*this, new FunDef(*this, code, 0, th.mCompileScope->numLocals(),
                                th.mCompileScope->numVars(), NULL));

  return true;
}

int TopCompileScope::directLookup(Thread& th, P<String> const& inName,
                                  size_t& outIndex, V& outBuiltIn) {
...
int CompileScope::directLookup(Thread& th, P<String> const& inName,
                               size_t& outIndex, V& outBuiltIn) {
...
int TopCompileScope::indirectLookup(Thread& th, P<String> const& inName,
                                    size_t& outIndex, V& outBuiltIn) {
  return directLookup(th, inName, outIndex, outBuiltIn);
}

int InnerCompileScope::indirectLookup(Thread& th, P<String> const& inName,
                                      size_t& outIndex, V& outBuiltIn) {
  ...
  if (scope == scopeUndefined) return scopeUndefined;
...
int TopCompileScope::bindVar(Thread& th, P<String> const& inName,
                             size_t& outIndex) {
  V v;
...
int CompileScope::innerBindVar(Thread& th, P<String> const& inName,
                               size_t& outIndex) {
  V v;
  int scope = directLookup(th, inName, outIndex, v);
...
int InnerCompileScope::bindVar(Thread& th, P<String> const& inName,
                               size_t& outIndex) {
...
int ParenCompileScope::directLookup(Thread& th, P<String> const& inName,
                                    size_t& outIndex, V& outBuiltIn) {
  return mNext->directLookup(th, inName, outIndex, outBuiltIn);
}

int ParenCompileScope::indirectLookup(Thread& th, P<String> const& inName,
                                      size_t& outIndex, V& outBuiltIn) {
  return mNext->indirectLookup(th, inName, outIndex, outBuiltIn);
}

int ParenCompileScope::innerBindVar(Thread& th, P<String> const& inName,
                                    size_t& outIndex) {
  return mNext->innerBindVar(th, inName, outIndex);
}

int ParenCompileScope::bindVar(Thread& th, P<String> const& inName,
                               size_t& outIndex) {
  return mNext->innerBindVar(th, inName, outIndex);
}
