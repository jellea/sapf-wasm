...
AUBuffers::AUBuffers(AudioBufferList *inIoData) : ioData(inIoData) {}

uint32_t AUBuffers::count() const { return this->ioData->mNumberBuffers; }
...
uint32_t RtBuffers::count() const { return this->theCount; }

float *RtBuffers::data(int channel) const {
  return this->out + channel * this->theSize;
}

uint32_t RtBuffers::size(int channel) const { return this->theSize; }
#endif