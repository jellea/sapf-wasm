...
[[noreturn]] void notFound(Arg key) {
  post("notFound ");
  key.print(gDummyThread);  // keys are either symbols or numbers neither of
                            // which will use the thread argument to print.
  post("\n");
  throw errNotFound;
}
...
Object::Object() : scratch(0), elemType(0), finite(false), flags(0) {
#if COLLECT_MINFO
...
V ZRef::deref() const { return z; }

Z Object::derefz() const { return asFloat(); }

Z ZRef::derefz() const { return z; }
...
GForm::GForm(P<GForm> const& inNext) : Object(), mNextForm(inNext) {
  mTable = new GTable();
}

P<GForm> consForm(P<GTable> const& inTable, P<GForm> const& inNext) {
  return new GForm(inTable, inNext);
}
P<Form> consForm(P<Table> const& inTable, P<Form> const& inNext) {
  return new Form(inTable, inNext);
}

void Object::apply(Thread& th) { th.push(this); }
...
Fun::Fun(Thread& th, FunDef* def) : mDef(def), mWorkspace(def->Workspace()) {
  if (NumVars()) {
...
FunDef::FunDef(Thread& th, P<Code> const& inCode, uint16_t inNumArgs,
               uint16_t inNumLocals, uint16_t inNumVars,
               P<String> const& inHelp)
    : mCode(inCode),
      mNumArgs(inNumArgs),
      mNumLocals(inNumLocals),
      mNumVars(inNumVars),
...
void Prim::apply(Thread& th) { apply_n(th, mTakes); }

void Prim::apply_n(Thread& th, size_t n) {
  if (th.stackDepth() < n) throw errStackUnderflow;
...
GTable* GTable::putPure(Arg inKey, int64_t inKeyHash, Arg inValue) {
  ...
  if (tree) {
    ...
    return new GTable(
        new TreeNode(inKey, inKeyHash, inValue, serialNo, nullptr, nullptr));
  }
}
...
TreeNode* TreeNode::putPure(Arg inKey, int64_t inKeyHash, Arg inValue) {
  ...
  if (inKeyHash < mHash) {
    if (left) {
      return new TreeNode(mKey, mHash, mValue, mSerialNumber,
                          left->putPure(inKey, inKeyHash, inValue), right);
    } else {
      int64_t serialNo = ++gTreeNodeSerialNumber;
      return new TreeNode(
          mKey, mHash, mValue, mSerialNumber,
          new TreeNode(inKey, inKeyHash, inValue, serialNo, nullptr, nullptr),
          right);
    }
  } else {
    if (right) {
      return new TreeNode(mKey, mHash, mValue, mSerialNumber, left,
                          right->putPure(inKey, inKeyHash, inValue));
    } else {
      int64_t serialNo = ++gTreeNodeSerialNumber;
      return new TreeNode(
          mKey, mHash, mValue, mSerialNumber, left,
          new TreeNode(inKey, inKeyHash, inValue, serialNo, nullptr, nullptr));
    }
  }
}
...
Gen::Gen(Thread& th, int inItemType, bool inFinite)
    : mDone(false),
      mOut(0),
      mBlockSize(inItemType == itemTypeV ? vm.VblockSize : th.rate.blockSize) {
  elemType = inItemType;
  setFinite(inFinite);
#if COLLECT_MINFO
...
void List::force(Thread& th) {
  ...
  if (mGen) {
    P<Gen> gen =
        mGen;  // keep the gen from being destroyed out from under pull().
    if (gen->done()) {
      gen->end();
    } else {
      gen->pull(th);
    }
    // mGen should be NULL at this point because one of the following should
    // have been called: fulfill, link, end.
  }
}

int64_t List::length(Thread& th) {
  if (!isFinite()) indefiniteOp("size", "");
...
void Array::alloc(int64_t inCap) {
  ...
  if (isV()) {
    V* oldv = vv;
    vv = new V[mCap];
    for (int64_t i = 0; i < size(); ++i) vv[i] = oldv[i];
    delete[] oldv;
  } else {
...
void Array::add(Arg inItem) {
  if (mSize >= mCap) alloc(2 * mCap);
  if (isV())
    vv[mSize++] = inItem;
  else
    zz[mSize++] = inItem.asFloat();
}
...
void Array::addAll(Array* a) {
  ...
  if (newSize > mCap) alloc(NEXTPOWEROFTWO(newSize));
...
In::In() : mList(nullptr), mOffset(0), mConstant(0.), mIsConstant(true) {}

VIn::VIn() { set(0.); }

VIn::VIn(Arg inValue) { set(inValue); }

ZIn::ZIn() { set(0.); }

ZIn::ZIn(Arg inValue) { set(inValue); }

BothIn::BothIn() { set(0.); }

BothIn::BothIn(Arg inValue) { set(inValue); }
...
void dumpList(List const* list) {
  for (int i = 0; list; ++i, list = list->nextp()) {
    ...
    printf("  List %d %p mArray %p %d\n", i, list, list->mArray(),
           (int)(list->mArray() ? list->mArray->size() : 0));
  }
}
...
bool ZIn::fillSegment(Thread& th, int inNum, Z* outBuffer) {
  ...
  while (list) {
    ...
    if (framesToFill == 0) return false;
...
class Comma : public Gen {
 ...
 public:
  Comma(Thread& th, Arg a, Arg inKey)
...
bool List::Equals(Thread& th, Arg v) {
  ...
  List* that = (List*)v.o();
  if (!isFinite()) indefiniteOp("", "equals : a");
  if (!that->isFinite()) indefiniteOp("", "equals : b");
...
List::List(P<Gen> const& inGen) : mNext(nullptr), mGen(inGen), mArray(0) {
...
TableMap::TableMap(size_t inSize) : mSize(inSize) {
  if (inSize == 0) {
    mMask = 0;
    mIndices = nullptr;
    mKeys = nullptr;
...
TableMap::TableMap(Arg inKey) : mSize(1) {
  mMask = 1;
...
bool Table::Equals(Thread& th, Arg v) {
  ...
  if (size != that->mMap->mSize) return false;
  for (size_t i = 0; i < size; ++i) {
    V key = mMap->mKeys[i];
    V thatValue;
    if (!that->getWithHash(th, key, key.Hash(), thatValue)) return false;
    if (!mValues[i].Equals(th, thatValue)) return false;
  }
  return true;
}
...
bool Table::getWithHash(Thread& th, Arg key, int64_t hash, V& value) const {
  size_t index;
  if (!mMap->getIndex(key, hash, index)) return false;
  value = mValues[index];
  return true;
}

void Table::put(size_t inIndex, Arg inValue) { mValues[inIndex] = inValue; }
