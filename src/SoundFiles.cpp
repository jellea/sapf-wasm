...
// SFReader::SFReader(std::unique_ptr<SoundFile> inSoundFile, int64_t
// inDuration) : 	mSoundFile(std::move(inSoundFile)),
...
// 		else {
// 			if (!output->mDummy)
// 				output->mDummy = (Z*)calloc(output->mBlockSize,
// sizeof(Z));
...
// th.rate.sampleRate); #else 	std::unique_ptr<SoundFile> soundFile =
// SoundFile::open(path, th.rate.sampleRate, th.rate.blockSize); #endif
...
// #ifdef SAPF_AUDIOTOOLBOX
// std::unique_ptr<SoundFile> sfcreate(Thread& th, const char* path, int
// numChannels, double fileSampleRate, bool interleaved)
// {
// 	return SoundFile::create(path, numChannels, th.rate.sampleRate,
// fileSampleRate, interleaved);
// }
// #else
// std::unique_ptr<SoundFile> sfcreate(Thread& th, const char* path, int
// numChannels, double fileSampleRate, bool interleaved, bool async)
// {
// 	return SoundFile::create(path, numChannels, th.rate.sampleRate,
// fileSampleRate, async);
// }
// #endif
...
// 				recDir = ".";
// 		}
// 		snprintf(path, len, "%s/%s.wav", recDir,
// ((String*)filename.o())->s); 	} else { 		int32_t count = ++gFileCount; 		#ifdef
// _WIN32 			const char* tempDir = getenv("TEMP"); 			if (!tempDir ||
// strlen(tempDir)==0) 				tempDir = getenv("TMP"); 			if (!tempDir ||
// strlen(tempDir)==0) 				tempDir = "."; 		#else 			const char* tempDir = "/tmp"; 		#endif
// 		snprintf(path, len, "%s/sapf-%s-%04d.wav", tempDir,
// gSessionTime, count);
...
// 	if (v.isZList()) {
// 		if (!v.isFinite()) indefiniteOp(">sf : s - indefinite number of
// frames", ""); 		numChannels = 1; 		in.push_back(ZIn(v)); 	} else { 		if
// (!v.isFinite()) indefiniteOp(">sf : s - indefinite number of channels", "");
// 		P<List> s = (List*)v.o();
// 		s = s->pack(th);
// 		Array* a = s->mArray();
...
// 		if (allIndefinite) indefiniteOp(">sf : s - all channels have
// indefinite number of frames", "");
// 	}
// 	v.o = nullptr;
...
// 	#ifdef SAPF_AUDIOTOOLBOX
// 		std::unique_ptr<SoundFile> soundFile = sfcreate(th, path,
// numChannels, 0., true); 	#else 		std::unique_ptr<SoundFile> soundFile =
// sfcreate(th, path, numChannels, 0., true, false); 	#endif 	if (!soundFile)
// return;
...
// #ifdef SAPF_AUDIOTOOLBOX
// 		OSStatus err = ExtAudioFileWrite(soundFile->mXAF, minn,
// bufs.abl); 		if (err) { 			post("file writing failed %d\n", (int)err); 			break;
// 		}
// #else
...
// framesWritten * th.rate.invSampleRate);
